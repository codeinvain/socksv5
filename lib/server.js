...
var socksClient = require('./client');
var AuthNone= require('./auth/None');
...
function Server(options, listener) {
  ...
  if (options && options.socksAgent){
    this.proxyHost = options.socksAgent.host;
    this.proxyPort = options.socksAgent.port;
  }
}
    ...
    function accept(intercept) {
      ...
      if (socket.writable) {
        if (intercept) {
          ...
          proxyOrChainSocket(self,socket, reqInfo);
        }
      }
    }
    function deny() {
...
Server.prototype.chainAuth = function(auth) {
  if (typeof auth !== 'object'
      || typeof auth.server !== 'function'
      || auth.server.length !== 2)
    throw new Error('Invalid authentication handler');
  else if (this._chainAuths.length >= 255)
    throw new Error('Too many authentication handlers (limited to 255).');

  this._chainAuths.push(auth);
...
function proxyOrChainSocket(server,socket,req){
  if (server.proxyHost && server.proxyPort){
    chainSocket(server,socket,req);
  }else{
    proxySocket(socket,req)
  }
}

function chainSocket(server,socket,req){
  var chainAuths = server._chainAuths;
  if (chainAuths.length==0){
    chainAuths.push(AuthNone());
  }
  var options = {
    host: req.dstAddr,
    port: req.dstPort,
  ...
  var client = socksClient.connect(options, function(dstSock) {
    if (socket.writable) {
...
function wireSockets(socket,dstSock,req){
}
